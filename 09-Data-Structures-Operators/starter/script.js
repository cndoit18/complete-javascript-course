...
// #4
let player1Final = [...player1, 'Thiago', 'Coutinho', 'Perisic'];
...
for (const [t, e] of gameEvents.entries()) {
  console.log(`[${t <= 45 ? 'FIRST HALF' : 'SECOND HALF'}] ${t}: ${e}`);
}
...
document.querySelector('button').addEventListener('click', () => {
  ...
  for (const [i, s] of text.split('\n').entries()) {
    if (s.length === 0) {
      continue;
    }
    console.log(convert(s).padEnd(20, ' ') + '✅'.repeat(i + 1));
  }
});

function convert(s) {
  let result = s.trim().split('_');
